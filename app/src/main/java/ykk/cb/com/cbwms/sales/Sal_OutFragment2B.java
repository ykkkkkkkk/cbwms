...
import ykk.cb.com.cbwms.sales.adapter.Sal_OutFragment2BAdapter;
 ...
 * 扫箱码 出库
 */
public class Sal_OutFragment2B extends BaseFragment implements IFragmentExec {
    ...
    private Sal_OutFragment2B context = this;
    ...
    private Sal_OutFragment2BAdapter mAdapter;
    ...
    private static class MyHandler extends Handler {
        private final WeakReference<Sal_OutFragment2B> mActivity;

        public MyHandler(Sal_OutFragment2B activity) {
            mActivity = new WeakReference<Sal_OutFragment2B>(activity);
        }

        public void handleMessage(Message msg) {
            final Sal_OutFragment2B m = mActivity.get();
            String errMsg = null;
            if (m != null) {
                m.hideLoadDialog();
...
//        listMaps.clear();
    ...
    public View setLayoutResID(LayoutInflater inflater, ViewGroup container) {
        return inflater.inflate(R.layout.sal_out_fragment2b, container, false);
    }
...
//        parent = (Sal_OutMainActivity) mContext;
        ...
        mAdapter = new Sal_OutFragment2BAdapter(mContext, checkDatas);
        recyclerView.setAdapter(mAdapter);
        //这个是让listview空间失去焦点
        recyclerView.setFocusable(false);
        mAdapter.setCallBack(new Sal_OutFragment2BAdapter.MyCallBack() {
            @Override
...
//        }
    ...
    private void getSourceAfter(List<MaterialBinningRecord> list) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if(deliOrder.getSaleOrgId() > 0) { // 用销售订单的查询的销售组织
                salOrg.setFpkId(deliOrder.getSaleOrgId());
                salOrg.setNumber(deliOrder.getSaleOrgNumber());
                salOrg.setName(deliOrder.getSaleOrgName());
            } else {
            ...
            if (customer == null) {
                ...
                customer.setFcustId(deliOrder.getCustId());
                customer.setCustomerCode(deliOrder.getCustNumber());
                customer.setCustomerName(deliOrder.getCustName());
...
//            sr2.setRelationObj(JsonUtil.objectToString(ism));
            ...
            srTok3.setSaleDeptNumber(deliOrder.getSalDeptNumber());
            srTok3.setSalerNumber(deliOrder.getSalerNumber());
            srTok3.setDeliverWayNumber(deliOrder.getDeliveryWayNumber());
            srTok3.setDeliveryCompanyNumber(deliOrder.getDeliveryCompanyNumber());
            srTok3.setFpaezHeadlocAddress(deliOrder.getReceiveAddress());
            srTok3.setFheadDeliveryWayNumber(deliOrder.getDeliveryWayNumber());
            srTok3.setFpaezRemark(isNULLS(deliOrder.getDeliRemark()));
            if(deptNumber == null) {
                deptNumber = deliOrder.getSalDeptNumber();
            }
            srTok3.setFdeliveryDeptNumber(deptNumber);
            srTok3.setFpaezContacts(deliOrder.getReceivePerson());
            srTok3.setFpaezContactnumber(deliOrder.getReceiveTel());
            srTok3.setFpaezTel(deliOrder.getReceiveTel());
            srTok3.setFdeliveryMethodNumber(deliOrder.getDeliveryMethodNumber());
...
//                .post(body)
    ...
    private void run_smGetDatas(String val) {
        ...
        switch (curViewFlag) {
            case '1': // 箱子扫码
                mUrl = getURL("materialBinningRecord/findList4ByParam");
                barcode = boxBarcode;
                strCaseId = "";
                break;
        }
